...
const AngleDisplay: React.FC<Angles> = ({
  ...
  const layers = [
    { angle: sunPosition, color: "orange", label: "Posição do Sol", radius: 120, icon: function(x:number=0, y:number=0) { return sunIcon(x,y)} },
    { angle: lensAngle,  color: "deepskyblue" , label: "Ângulo da Lente", radius: 85, icon: function(x:number=0, y:number=0) { return lensIcon(x,y)} },
    ...
    <svg width={435} height={260}>
      ...
      {layers.map(({ angle, color, radius, icon}, idx) => {
        ...
        return (
          <g key={idx}>   
             {icon(x,y)}
            <text x={x-5} y={y - 15} fontSize={10} fill={color}>
              {angle + "\u00B0"}
            </text>
          </g>
        );
